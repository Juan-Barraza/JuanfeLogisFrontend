...
import { useCreateLocation, useUpdateLocation } from '@/hooks/useSettings'
import type { Location } from '@/types/settings.types'

interface LocationModalProps {
    isOpen: boolean
    onClose: () => void
    locationToEdit?: Location | null // Si es null o undefined, estamos creando
}
...
export default function LocationModal({ isOpen, onClose, locationToEdit }: LocationModalProps) {
    ...
    useEffect(() => {
        if (locationToEdit) {
            reset({ name: locationToEdit.name })
        } else {
            reset({ name: '' })
        }
    }, [locationToEdit, reset, isOpen])
            ...
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="px-6 py-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-800">
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">
                        {locationToEdit ? 'Editar Bodega' : 'Nueva Bodega'}
                    </h3>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 overflow-y-auto">
                    {(createMutation.error || updateMutation.error) && (
                        <div className="mb-4 p-3 rounded-lg bg-red-50 border border-red-100 text-red-600 text-sm">
                            {(createMutation.error as any)?.response?.data?.error || (updateMutation.error as any)?.response?.data?.error || 'Error al guardar la bodega'}
                        </div>
                    )}
